...
async function getProjects() {
  ...
  const projects = Object.entries(map).map(([name, ctrs], i) => {
    ...
    return { id: encodeURIComponent(name), name, description:null, budget:null, currency:'NGN',
              ...
              <Link key={proj.id} href={href} className="block group">
                <div className="rounded-2xl overflow-hidden transition-all hover:shadow-lg hover:-translate-y-0.5" style={{ background:'#FFFFFF', border:'1px solid #E2E8F0' }}>
                  ...
                  <div className="px-6 pt-5 pb-4">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="text-right shrink-0">
                        ...
                        {proj.budget
                          ? <p className="text-xs mt-0.5" style={{ color:'#94A3B8' }}>of {formatCurrency(proj.budget, proj.currency||'NGN')}</p>
                  ...
                  <div className="grid grid-cols-3 divide-x divide-[#F1F5F9]" style={{ borderTop:'1px solid #F1F5F9', background:'#FAFBFC' }}>
                    {[
                      { label:'Committed', value:formatCurrency(proj.committed, proj.currency||'NGN'), color:'#0F172A' },
                      { label:'Paid',      value:formatCurrency(proj.paid,      proj.currency||'NGN'), color:'#10B981' },
                      { label:'Balance',   value:formatCurrency(proj.committed-proj.paid, proj.currency||'NGN'), color:proj.committed-proj.paid>0?'#F59E0B':'#94A3B8' },
                    ].map(s=>(

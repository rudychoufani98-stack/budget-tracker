    ...
    <div className="px-6 py-8 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <h1 className="text-2xl font-medium" style={{ color:'#0F172A' }}>Invoices</h1>
        </div>
      ...
      <div className="flex gap-3 mb-6 flex-wrap">
        {[
          ...
          <div key={p.label} className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium" style={{ background:'#E2E8F0', border:'1px solid #CBD5E1', color:p.color }}>
            {p.label} <span className="px-1.5 py-0.5 rounded-full text-xs" style={{ background:'#CBD5E1' }}>{p.count}</span>
          </div>
        ))}
      </div>
            ...
            {invoices.map((inv) => {
              ...
              return (
                <Link key={inv.id} href={`/invoices/${inv.id}`} className="grid px-6 py-3 hover:bg-white/5 transition-colors items-center" style={{ borderBottom:`1px solid ${C.border}`, gridTemplateColumns:'0.7fr 1.5fr 1.5fr 1fr 1fr 1fr 1.5fr 0.5fr' }}>
                  ...
                  <div>
                    <p className="text-sm" style={{ color:'#0F172A' }}>{(inv as any).service_providers?.name || inv.subcontractor_name || '—'}</p>
                  ...
                  <div className="text-sm" style={{ color:'#0F172A' }}>{formatCurrency(inv.amount_ht)}</div>
                  <div className="text-sm" style={{ color:C.muted }}>{formatCurrency(inv.amount_tva)}</div>
                  <div className="text-sm font-medium" style={{ color:'#0F172A' }}>{formatCurrency(inv.amount_ttc)}</div>
                  <div>
                    <span className="text-xs px-2.5 py-1 rounded-full" style={{ background:`${st.color}20`, color:st.color }}>{st.label}</span>
                  </div>
                  <div className="flex justify-end">
                    <svg width="14" height="14" fill="none" stroke="#94A3B8" strokeWidth="2" viewBox="0 0 24 24"><polyline points="9 18 15 12 9 6"/></svg>
                  </div>
                </Link>
              )
            })}

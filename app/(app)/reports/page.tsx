    ...
    <div className="px-6 py-8 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <h1 className="text-2xl font-medium" style={{ color:'#F9FAFB' }}>Reports</h1>
        </div>
        <button onClick={()=>window.print()} className="text-sm font-medium px-4 py-2 rounded-xl" style={{ background:'#1F2937', border:'1px solid #374151', color:'#F9FAFB' }}>Print / PDF</button>
      </div>
      ...
      {tab === 'provider' && (
        <div>
          <div className="flex justify-between mb-4">
            <p className="text-sm font-medium" style={{ color:'#F9FAFB' }}>Payment Summary by Provider</p>
            <button onClick={()=>exportCSV([['Provider','Contracted','Paid','Balance'],...(data.byProvider||[]).map((p:any)=>[p.name,p.total_contracted,p.total_paid,p.total_contracted-p.total_paid])],'providers.csv')} className="text-xs px-3 py-1.5 rounded-lg" style={{ background:'#1F2937', color:C.blue }}>Export CSV</button>
          </div>
            ...
            {(data.byProvider||[]).map((p:any) => {
              ...
              return (
                <div key={p.name} className="grid px-6 py-3 items-center" style={{ borderBottom:`1px solid ${C.border}`, gridTemplateColumns:'2fr 1fr 1fr 1fr 1fr' }}>
                  <p className="text-sm" style={{ color:'#F9FAFB' }}>{p.name}</p>
                  <p className="text-sm" style={{ color:'#F9FAFB' }}>{formatCurrency(p.total_contracted)}</p>
      ...
      {tab === 'category' && (
        <div>
          <p className="text-sm font-medium mb-4" style={{ color:'#F9FAFB' }}>Payment Summary by ESG Category</p>
          <div className="rounded-2xl overflow-hidden" style={{ background:C.card, border:`1px solid ${C.border}` }}>
            ...
            {(data.byCategory||[]).map((c:any) => (
              <div key={c.category} className="grid px-6 py-3" style={{ borderBottom:`1px solid ${C.border}`, gridTemplateColumns:'1fr 1fr 1fr 1fr' }}>
                <span className="text-sm font-medium" style={{ color:'#F9FAFB' }}>{c.category}</span>
                <span className="text-sm" style={{ color:'#F9FAFB' }}>{formatCurrency(c.total)}</span>
      ...
      {tab === 'monthly' && (
        <div>
          <p className="text-sm font-medium mb-4" style={{ color:'#F9FAFB' }}>Monthly Payment Flow (approved invoices)</p>
      ...
      {tab === 'vat' && (
        <div>
          <p className="text-sm font-medium mb-4" style={{ color:'#F9FAFB' }}>VAT Summary (all approved invoices)</p>
      ...
      {tab === 'audit' && (
        <div>
          <p className="text-sm font-medium mb-4" style={{ color:'#F9FAFB' }}>Audit Log (last 200 actions)</p>
          <div className="rounded-2xl overflow-hidden" style={{ background:C.card, border:`1px solid ${C.border}` }}>
            ...
            {(data.auditLog||[]).map((a:any)=>(
              <div key={a.id} className="grid px-6 py-2.5 items-center text-sm" style={{ borderBottom:`1px solid ${C.border}`, gridTemplateColumns:'2fr 1fr 1fr 2fr' }}>
                <span style={{ color:'#F9FAFB' }}>{a.action}</span>
                <span style={{ color:C.muted }}>{a.entity_type}</span>
                <span style={{ color:C.muted }}>{new Date(a.created_at).toLocaleDateString('fr-FR')}</span>
                <span className="text-xs font-mono truncate" style={{ color:'#4B5563' }}>{a.details ? JSON.stringify(a.details).slice(0,60) : '—'}</span>
              </div>
            ))}
          </div>
        </div>

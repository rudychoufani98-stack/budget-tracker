...
export function ContractsClient({ contracts, projects, initialProject, initialSection }: Props) {
  ...
  const totalValue = filtered.reduce((s:number,c:any)=>s+(c.contract_amount||c.total_budget||0),0)
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          { label:'Total Value',    value:formatCurrency(totalValue), sub:`${filtered.length} contracts`,   color:'#3B82F6' },
          { label:'Total Paid',     value:formatCurrency(totalPaid),  sub:'across all tranches',             color:'#10B981' },
          { label:'Balance',        value:formatCurrency(totalValue-totalPaid), sub:'remaining to pay',     color:'#F59E0B' },
        ].map(k=>(
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background:'#FFFFFF', border:'1px solid #E2E8F0' }}>
        <div className="grid px-6 py-3 text-xs font-semibold uppercase tracking-widest" style={{ color:'#94A3B8', borderBottom:'1px solid #F1F5F9', background:'#FAFBFC', gridTemplateColumns:'2fr 1.2fr 1.2fr 0.6fr 1fr 1fr 1fr 1.4fr 0.5fr' }}>
          <div>Contract</div>
          <div>Project</div>
          <div>Section</div>
          <div>Cat</div>
        ...
        {filtered.length === 0 ? (
          <div className="py-16 text-center">
            ...
            {!hasFilter && (
              <Link href="/contracts/new" className="inline-flex mt-4 text-sm font-semibold px-4 py-2 rounded-xl" style={{ background:'#3B82F6', color:'#fff' }}>+ New Contract</Link>
            )}
          </div>
          ...
          return (
            <Link
              ...
              style={{ borderBottom:'1px solid #F8FAFC', gridTemplateColumns:'2fr 1.2fr 1.2fr 0.6fr 1fr 1fr 1fr 1.4fr 0.5fr' }}
            >
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {scheduled>0 && budget>0 && <span className="text-xs" style={{ color:'#94A3B8' }}>+{Math.round(scheduled/budget*100)}% sched.</span>}
                </div>

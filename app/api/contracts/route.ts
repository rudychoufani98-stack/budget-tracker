...
export async function GET() {
  const { data, error } = await supabaseAdmin
    .from('contracts')
    .select('*, service_providers(*), contract_tranches(*)')
...
export async function POST(req: NextRequest) {
  ...
  const { contract_name, client_name, service_provider_id, project, category,
  ...
  const { data, error } = await supabaseAdmin.from('contracts').insert({
    contract_name, client_name, service_provider_id, project, category,

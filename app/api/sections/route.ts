...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .insert({ project_id, name: name.trim(), description: description||null, budget: budget||null, currency: currency||'NGN', start_date: start_date||null, end_date: end_date||null, status: status||'active' })
    .select().single()

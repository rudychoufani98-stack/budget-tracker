...
function tpl({ title, body, comment, link, linkLabel }: {
  ...
  <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;background:#F8FAFC;border-radius:12px">
    <div style="background:#FFFFFF;border-radius:8px;padding:24px">
      ...
      <p style="color:#64748B;margin:0 0 16px">${body}</p>
      ${comment ? `<div style="background:#E2E8F0;border-left:3px solid #3B82F6;padding:10px 14px;border-radius:4px;margin-bottom:16px"><p style="margin:0;color:#475569;font-style:italic">"${comment}"</p></div>` : ''}
...
export async function sendBudgetAlertEmail(contractId: string, contractName: string, pct: number) {
  ...
  await resend.emails.send({
    from: FROM, to: process.env.RUDY_EMAIL,
    subject: `Budget alert - ${contractName} at ${Math.round(pct)}%`,
...
export async function sendTrancheReminderEmail(contractName: string, trancheName: string, amount: number, daysUntil: number, contractId: string) {
  ...
  await resend.emails.send({
    ...
    html: tpl({ title: 'Upcoming Tranche Payment', body: `Tranche <strong>${trancheName}</strong> of contract <strong>${contractName}</strong> (${amount.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}) is due in <strong>${daysUntil} days</strong>.`, link: `${APP}/contracts/${contractId}`, linkLabel: 'View Contract' }),
  })
}
